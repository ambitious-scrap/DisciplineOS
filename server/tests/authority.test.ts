...
function seedBalance(userId: string, seconds: number) {
  const bank = db.timeBanks.get(userId);
  if (!bank) throw new Error('Test time bank missing');
...
describe('Backend authority invariants', () => {
  ...
  it('does not double-credit a repeated manual task idempotency key', async () => {
    ...
    const task = await app.request('/api/tasks', {
      ...
      body: JSON.stringify({ title: 'Idempotent task', evidenceType: 'none' }),
    });
    ...
    expect((await balance.json()).balanceSeconds).toBe(300);
  });

  it('does not double-spend a repeated transaction idempotency key', async () => {
    const { token, userId, deviceId } = await registerAndPair('spend-idempotency@disciplineos.local');
    seedBalance(userId, 1800);
    const headers = { 'Content-Type': 'application/json', Authorization: `Bearer ${token}` };
    const body = JSON.stringify({ seconds: 1200, targetType: 'app', targetIdentifier: 'com.example.app', deviceId, idempotencyKey: 'spend-idempotency-1' });
  ...
  it('serializes concurrent spends and never makes the balance negative', async () => {
    const { token, userId, deviceId } = await registerAndPair('concurrent-spend@disciplineos.local');
    seedBalance(userId, 1800);
    const headers = { 'Content-Type': 'application/json', Authorization: `Bearer ${token}` };
  ...
  it('allows only one concurrent active unlock session', async () => {
    const { token, userId, deviceId } = await registerAndPair('concurrent-session@disciplineos.local');
    seedBalance(userId, 1800);
    const headers = { 'Content-Type': 'application/json', Authorization: `Bearer ${token}` };
